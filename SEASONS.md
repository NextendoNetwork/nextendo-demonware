...
- When a new season is announced, add its theme to `season_themes` in `pubfiles.json` (see [PUBFILES.md](PUBFILES.md)); no code change is needed, and the rotation picks it up in numeric order. The built-in table is in `seasons.go` (`originalThemes` and `recycledThemes`) and can be extended there too.
